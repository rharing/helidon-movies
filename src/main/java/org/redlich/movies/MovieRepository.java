...
public class MovieRepository {
    ...
    Map<Integer, Movie> data = new ConcurrentHashMap<>();
    ...
    public Movie save(Movie movie) {
        if (movie.getId() == null) {
            movie.setId(data.size() + 1);
        }
        data.put(movie.getId(), movie);
        return movie;
    }
    public Boolean exists(Movie existingMovie){
        for(Movie movie : data.values()) {
            if(movie.getTitle().equals(existingMovie.getTitle()) && movie.getYear() == existingMovie.getYear())
                return true;
        }
        return false;
    }
